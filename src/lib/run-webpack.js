...
export default (watch = false, config, onprogress) =>
    ...
    if (watch) {
      ...
      compiler.plugin("done", stats => {
        if (first) {
          ...
          let serverAddr = `${devServer.https === true
            ? "https"
            : "http"}://${process.env.HOST ||
            devServer.host ||
            "localhost"}:${process.env.PORT || devServer.port || 8080}`;
          process.stdout.write(
            `  \u001b[32m> Development server started at ${serverAddr}\u001b[39m\n`
          );
        }
...
export function writeJsonStats(stats) {
  ...
  return fs.writeFile(outputPath, JSON.stringify(jsonStats)).then(() => {
